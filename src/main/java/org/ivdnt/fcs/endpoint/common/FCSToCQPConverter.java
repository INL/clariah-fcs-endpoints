...
public class FCSToCQPConverter {
	 ...
	 *             If the query is too complex or it for any other reason cannot be
	 *             performed
	 */
	public static String makeCQPFromCQL(final SRUQuery<CQLNode> query) throws SRUException {
		...
		/*
		 * Translate the CQL query to a Lucene query. If a CQL feature was used, that is
		 * not supported by us, throw a SRU error (with a detailed error message)
		 *
		...
		if (node instanceof CQLBooleanNode) {
			...
			if (terms.length > 1) {
				...
				for (int i = 0; i < terms.length; i++) {
					if (terms[i].startsWith("\"") || terms[i].startsWith("'") || terms[i].endsWith("\"")
							|| terms[i].endsWith("'")) {
						String tmp = terms[i];
			...
			throw new SRUException(SRUConstants.SRU_CANNOT_PROCESS_QUERY_REASON_UNKNOWN, "unknown cql node: " + node);
		}
	}
