...
public class CqpWriter {
	 ...
	 * @param node
	 *            (expression node)
	 * @return expression as CQL (<i>name='value'</i> if grammatical features are
	 *         indexed separately, or else <i>pos=some regex</i>)
	 */
	private String writeExpression(Expression node) {
		...
		if (this.useRegex && this.grammaticalFeatures.contains(n)) {
			if (this.includeFeatureNameInRegex) {
				...
				return String.format("%s%s%s.*%s=(%s).*%s", posTagFeature, operator, valueQuote, n, valueMatch,
						valueQuote);
			} else
				return String.format("%s%s%s.*[\\(,\\|](%s)[,\\)\\|].*%s", posTagFeature, operator, valueQuote, v,
						valueQuote);
		} else
